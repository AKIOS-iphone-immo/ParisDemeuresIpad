...
@interface AFOpenFlowViewDiapo (hidden)
...
@implementation AFOpenFlowViewDiapo (hidden)
...
@implementation AFOpenFlowViewDiapo
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
	if (isSingleTap) {
		...
		if (targetCover && (targetCover.number != selectedCoverView.number)){
            ...
            NSLog(@"DIAPORAMA FICHE DETAILLEE");
            [[NSNotificationCenter defaultCenter] postNotificationName:@"coverFlowFicheDetaillee" object: [NSNumber numberWithInt:selectedCoverView.number]];
        }
	}
	[self centerOnSelectedCover:YES];
