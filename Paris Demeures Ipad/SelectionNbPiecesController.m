...
- (void)viewDidLoad {
    ...
    NSUInteger indexArr[] = {0,[nb_pieces_mini intValue] - 1};
    
    NSIndexPath *path = [NSIndexPath indexPathWithIndexes:indexArr length:2];
    [rowSelected addObject:path];
    [rowSelectedValue addObject:@""];
    
    NSUInteger indexArr2[] = {0,[nb_pieces_maxi intValue] - 1};
    
    path = [NSIndexPath indexPathWithIndexes:indexArr2 length:2];
    [rowSelected addObject:path];
    [rowSelectedValue addObject:@""];
    ...
    [boutonRetour setFrame:CGRectMake(600,875,100,60)];
...
- (void) setCriteres:(NSNotification *)notify
{
    ...
    nb_pieces_mini = [criteres valueForKey:@"nb_pieces_mini"];
    nb_pieces_maxi = [criteres valueForKey:@"nb_pieces_maxi"];
    
}

- (void)viewWillAppear:(BOOL)animated {
	//[rowSelected removeAllObjects];
	//[rowSelectedValue removeAllObjects];
...
- (void)viewWillDisappear:(BOOL)animated {
	[[NSNotificationCenter defaultCenter] postNotificationName:@"nbPiecesSelected" object:rowSelected];
